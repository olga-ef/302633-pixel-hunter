...
const answeres = Array.from(screen.querySelectorAll(`.game__answer > input`));
const optionsNumber = screen.querySelectorAll(`.game__option`).length;
const backButton = screen.querySelector(`.back`);

const isAllAnsweres = () => {
  const selectedAnsweres = answeres.filter((answer) => answer.checked);
  return selectedAnsweres.length === optionsNumber;
};
